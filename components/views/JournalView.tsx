        ...
        <div className="flex gap-2">
            {moods.map((m) => (
                <button
                    ...
                    title={m.label}
                    className={`p-2 rounded-lg transition-all border ${current === m.id
...
const JournalView: React.FC = () => {
    // State
    const [entries, setEntries] = useStorage<JournalEntry[]>('p67_journal', INITIAL_ENTRIES);
                ...
                <div className="flex-1 overflow-y-auto p-2 space-y-2 scrollbar-thin">
                    ...
                    {entries.map(entry => (
                        ...
                        >
                            ...
                            {/* Mood Indicator Dot */}
                            <div className={`absolute top-3 right-3 w-2 h-2 rounded-full ${entry.mood === 'HAPPY' ? 'bg-green-500' :
