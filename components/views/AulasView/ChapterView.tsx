...
import { ChevronLeft, ImagePlus, X, Edit, Eye, Save, Trash2, Bookmark, Highlighter, CheckCircle, CheckCircle2, ClipboardCopy, FileJson, MessageSquare, Check, RotateCcw } from "lucide-react";
...
export default function ChapterView({ bookId, chapterId, onBack }: ChapterViewProps) {
  ...
  const togglePrincipalQuestionDone = (questionNumber: number) => {
    if (!book || !chapter) return;

    const completedQuestions = chapter.completedPrincipalQuestions || [];
    const isCompleted = completedQuestions.includes(questionNumber);
    const nextCompletedQuestions = isCompleted
      ? completedQuestions.filter((item) => item !== questionNumber)
      : [...completedQuestions, questionNumber].sort((a, b) => a - b);

    updateChapter(book.id, chapter.id, {
      completedPrincipalQuestions: nextCompletedQuestions,
    });
  };
      ...
      <header className="h-14 border-b border-slate-900 flex items-center px-4 bg-slate-950 shrink-0 justify-between gap-2 overflow-x-auto">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <motion.button
            ...
            whileTap={{ scale: 0.94 }}
            animate={lessonPromptCopied ? { scale: [1, 1.06, 1] } : { scale: 1 }}
            transition={{ duration: 0.24, ease: "easeOut" }}
            className={cn(
              "text-[9px] font-bold uppercase tracking-widest px-2.5 py-1.5 rounded flex items-center gap-1 transition-colors shadow-sm cursor-pointer",
              lessonPromptCopied
                ? "bg-emerald-500/20 border border-emerald-500/40 text-emerald-400"
                : "bg-slate-900 border border-slate-800 hover:bg-slate-850 text-slate-200"
            )}
            title="Copiar prompt para criar esta aula em Markdown"
          >
            <ClipboardCopy className="w-3.5 h-3.5" /> Prompt Aula
          </motion.button>
          <motion.button
            onClick={copyQuestionPrompt}
            whileTap={{ scale: 0.94 }}
            animate={questionPromptCopied ? { scale: [1, 1.06, 1] } : { scale: 1 }}
            transition={{ duration: 0.24, ease: "easeOut" }}
            className={cn(
              "text-[9px] font-bold uppercase tracking-widest px-2.5 py-1.5 rounded flex items-center gap-1 transition-colors shadow-sm cursor-pointer",
              questionPromptCopied
                ? "bg-emerald-500/20 border border-emerald-500/40 text-emerald-400"
                : "bg-slate-900 border border-slate-800 hover:bg-slate-850 text-slate-200"
            )}
            title="Copiar prompt para gerar JSON de questões relacionadas"
          >
            <ClipboardCopy className="w-3.5 h-3.5" /> Prompt Questões
          </motion.button>
          <button
            onClick={() => setQuestionsJsonDialogOpen(true)}
            className="text-[9px] font-bold uppercase tracking-widest bg-slate-900 border border-slate-800 hover:bg-slate-850 text-slate-200 px-2.5 py-1.5 rounded flex items-center gap-1 transition-colors shadow-sm cursor-pointer"
            title="Colar JSON de questões nesta aula"
          >
            <FileJson className="w-3.5 h-3.5" /> JSON Questões
          </button>
          <button
          ...
          >
            {chapter.readAt ? <CheckCircle className="w-3.5 h-3.5" /> : <CheckCircle2 className="w-3.5 h-3.5" />}
            {chapter.readAt ? "Lida" : "Marcar Lida"}
          </button>

          {isEditingContent ? (
            <button
              onClick={saveContent}
              className="text-[9px] font-bold uppercase tracking-widest bg-[#D4AF37] hover:bg-[#C2A032] text-slate-950 px-2.5 py-1.5 rounded flex items-center gap-1 transition-colors shadow-sm cursor-pointer"
            >
            ...
            <button
              ...
              className="text-[9px] font-bold uppercase tracking-widest bg-slate-900 border border-slate-800 hover:bg-slate-850 text-slate-200 px-2.5 py-1.5 rounded flex items-center gap-1 transition-colors shadow-sm cursor-pointer"
            >
              <Edit className="w-3.5 h-3.5" /> Editar MD
            </button>
          )}
          ...
          {!isEditingContent && (
            ...
            >
              {editMode ? <Eye className="w-3.5 h-3.5" /> : <ImagePlus className="w-3.5 h-3.5" />}
              {editMode ? "Leitura" : "Anexos"}
            </button>
          )}
        </div>
      </header>
          ...
          {isEditingContent ? (
            ...
            >
              ...
              {chapter.relatedQuestions && (
                <section className="not-prose bg-slate-900 border border-slate-800 rounded-lg p-5 mb-8 shadow-sm">
                  <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between mb-5">
                    <div>
                      <h2 className="text-xl font-serif italic text-slate-100">Questões Relacionadas</h2>
                      <p className="text-xs text-slate-400 mt-1">
                        Aula {chapter.relatedQuestions.aula} — {chapter.relatedQuestions.titulo}
                      </p>
                    </div>
                    <div className="text-[10px] uppercase tracking-widest text-[#D4AF37] border border-[#D4AF37]/30 rounded px-2 py-1 bg-slate-950 max-w-max">
                      {chapter.relatedQuestions.questoes_principais?.length || 0} Principais
                    </div>
                  </div>
                  ...
                  <div className="mb-5">
                    ...
                    {chapter.relatedQuestions.questoes_principais?.length > 0 && (
                      <p className="mb-3 text-[10px] uppercase tracking-widest text-slate-500 font-sans">
                        {(chapter.completedPrincipalQuestions || []).filter((q) =>
                          chapter.relatedQuestions?.questoes_principais.includes(q)
                        ).length}{" "}
                        / {chapter.relatedQuestions.questoes_principais.length} feitas
                      </p>
                    )}
                    <div className="flex flex-wrap gap-2">
                      {chapter.relatedQuestions.questoes_principais?.length > 0 ? (
                        chapter.relatedQuestions.questoes_principais.map((questionNumber) => {
                          const isCompleted = (chapter.completedPrincipalQuestions || []).includes(
                            questionNumber
                          );
                          ...
                          return (
                            <button
                              ...
                              type="button"
                              onClick={() => togglePrincipalQuestionDone(questionNumber)}
                              aria-pressed={isCompleted}
                              title={isCompleted ? "Marcar como pendente" : "Marcar como resolvida"}
                              className={cn(
                                "inline-flex min-w-9 items-center justify-center gap-1.5 rounded border px-2.5 py-1 text-xs font-bold transition-colors focus:outline-none focus:ring-2 focus:ring-[#D4AF37]/50 cursor-pointer",
                                isCompleted
                                  ? "border-[#D4AF37]/30 bg-slate-950 text-[#D4AF37]/70 line-through decoration-[#D4AF37]/50"
                                  : "border-[#D4AF37] bg-[#D4AF37] text-slate-950 hover:bg-[#F3D76F]"
                              )}
                            >
                  ...
                  {chapter.relatedQuestions.por_secao && chapter.relatedQuestions.por_secao.length > 0 && (
                    <div className="space-y-4 mb-5">
                      ...
                      {chapter.relatedQuestions.por_secao.map((section) => (
                        <div key={section.secao} className="border-l-2 border-slate-800 pl-4">
                          <div className="text-sm font-semibold text-slate-200 mb-2">{section.secao}</div>
                          <div className="flex flex-wrap gap-2">
                            {section.questoes.map((questionNumber) => (
                              <span
                                key={`${section.secao}-${questionNumber}`}
                                className="inline-flex min-w-8 justify-center rounded border border-slate-800 bg-slate-950 px-2 py-1 text-xs font-semibold text-slate-300"
                              >
                                {questionNumber}
                              </span>
                            ))}
                          ...
                          {chapter.relatedQuestions.questoes_secundarias_que_misturam_com_aulas_futuras.map(
                            (questionNumber) => (
                              <span
                                ...
                                className="inline-flex min-w-8 justify-center rounded border border-slate-850 bg-slate-900 px-2 py-1 text-xs font-semibold text-slate-400"
                              >
                                {questionNumber}
                              </span>
                            )
                  ...
                  {chapter.relatedQuestions.observacao && (
                    <p className="text-xs leading-relaxed text-slate-400 border-t border-slate-850 pt-4">
                      <strong>Observações:</strong> {chapter.relatedQuestions.observacao}
                    </p>
                  )}
                </section>
              )}
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {openComments.map((comment) => (
                <div
                  key={comment.id}
                  className={cn(
                    "rounded border p-3 bg-slate-900 transition-colors",
                ...
                >
                  <button type="button" onClick={() => setActiveCommentId(comment.id)} className="block w-full text-left cursor-pointer">
                    <p className="text-[11px] leading-relaxed text-slate-400 line-clamp-2 border-l-2 border-blue-500 pl-2 mb-3">
                      "{comment.selectedText}"
                    </p>
                    <p className="text-sm leading-relaxed text-slate-200 whitespace-pre-wrap">{comment.body}</p>
                  </button>
                  <div className="flex items-center justify-end gap-2 mt-3">
                    <button
                      ...
                      className="p-1.5 rounded border border-slate-800 text-emerald-500 hover:bg-emerald-500/10 cursor-pointer"
                    >
