...
const LinksView: React.FC = () => {
  // --- STATE ---
  const [links, setLinks] = useLocalStorage<LinkItem[]>('p67_links', INITIAL_LINKS);
  ...
  const handleDragOver = (e: React.DragEvent, targetItem: LinkItem) => {
    ...
    setLinks(prev => {
      ...
      // Update order property for all items in this category to match array index
  ...
  return (
    <div className="max-w-6xl mx-auto pb-20 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-8">
        <div className="w-full md:w-auto flex flex-col gap-1">
           <h2 className="text-3xl font-bold text-white flex items-center gap-2">
             <Globe className="text-indigo-400" /> Central de Links
           </h2>
           <p className="text-slate-400 text-sm">Gerencie seus atalhos e portais favoritos.</p>
        </div>
      ...
      {/* TABS */}
      <div className="flex bg-slate-800/50 p-1.5 rounded-2xl border border-slate-700 mb-8 w-full max-w-md mx-auto md:mx-0">
         <button 
            ...
            >
               ...
               <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-500 truncate flex items-center gap-1">
                     {new URL(formatUrl(link.url)).hostname.replace('www.', '')}
               ...
               <div className="absolute bottom-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity" onClick={e => e.stopPropagation()}>
                  <div className="flex gap-1">
                     <button onClick={() => { setEditingLink(link); setIsModalOpen(true); }} className="p-1.5 bg-slate-900 hover:bg-indigo-600 hover:text-white text-slate-400 rounded-lg transition-colors border border-slate-700">
                        <Edit2 size={14} />
                     </button>
                     <button onClick={() => handleDelete(link.id)} className="p-1.5 bg-slate-900 hover:bg-red-600 hover:text-white text-slate-400 rounded-lg transition-colors border border-slate-700">
                        <Trash2 size={14} />
                     </button>
                  </div>
               </div>
...
const LinkModal: React.FC<{ link: LinkItem | null, onClose: () => void, onSave: (data: any) => void }> = ({ link, onClose, onSave }) => {
            ...
            <div className="p-6 space-y-4">
               ...
               <div>
                   ...
                   <div className="flex gap-2">
                       <button 
                         onClick={() => setFormData({...formData, category: 'PERSONAL' as any})}
                       ...
                       <button 
                         onClick={() => setFormData({...formData, category: 'GENERAL' as any})}
