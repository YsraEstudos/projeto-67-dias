...
import { MessageSquare, Folder, Plus, FileText, ArrowLeft } from 'lucide-react';
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-4 bg-slate-800/30 p-6 rounded-3xl border border-slate-700/50">
               <div>
                  <button 
                     ...
                     className="text-slate-400 hover:text-white flex items-center gap-2 mb-2 text-sm font-medium transition-colors"
                  >
                     <ArrowLeft size={16} /> Voltar
                  </button>
                  <h3 className="text-xl font-bold text-emerald-400 flex items-center gap-2">
                     <Folder size={24} className="fill-emerald-500/20" />
                     {selectedFolder}
                  </h3>
               </div>
               <button className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl shadow-lg shadow-emerald-900/20 font-bold flex items-center gap-2 transition-all">
                  <Plus size={18} /> Novo Arquivo Markdown
               </button>
            </div>

            {/* FOLDER CONTENT MOCKUP */}
            <div className="flex flex-col gap-2">
               <div className="flex items-center justify-between p-4 bg-slate-800/50 border border-slate-700 rounded-xl hover:bg-slate-800 transition-colors cursor-pointer group">
                  <div className="flex items-center gap-3">
                     ...
                     <span className="text-slate-300 font-medium">Documento de Teste.md</span>
                  </div>
               ...
               <div className="flex items-center justify-between p-4 bg-slate-800/50 border border-slate-700 rounded-xl hover:bg-slate-800 transition-colors cursor-pointer group">
                  <div className="flex items-center gap-3">
                     <FileText size={18} className="text-slate-500 group-hover:text-emerald-400 transition-colors" />
                     <span className="text-slate-300 font-medium">Anotações.md</span>
                  </div>
   ...
   return (
      <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
         ...
         <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            <div 
               onClick={() => setSelectedFolder("Ideias & Projetos")}
               className="bg-slate-800/40 border border-slate-700 hover:border-emerald-500/50 p-4 rounded-2xl cursor-pointer transition-all flex flex-col gap-3 group"
            >
               <div className="flex items-center gap-3">
            ...
            <div 
               onClick={() => setSelectedFolder("Reflexões")}
               className="bg-slate-800/40 border border-slate-700 hover:border-emerald-500/50 p-4 rounded-2xl cursor-pointer transition-all flex flex-col gap-3 group"
            >
               <div className="flex items-center gap-3">
                  <div className="p-2.5 bg-slate-700 group-hover:bg-emerald-500/20 rounded-xl text-slate-400 group-hover:text-emerald-400 transition-colors">
            ...
            <div 
               onClick={() => alert("Simulação: Criar nova pasta!")}
               className="border-2 border-dashed border-slate-700 hover:border-emerald-500/50 p-4 rounded-2xl cursor-pointer transition-all flex items-center justify-center gap-2 text-slate-500 hover:text-emerald-400"
            >
               <Plus size={20} />
         ...
         <div className="mt-4">
            ...
            <div className="flex flex-col gap-2">
               <div 
                  onClick={() => alert("Simulação: Abrindo Planejamento do Aplicativo.md")}
                  className="flex items-center justify-between p-4 bg-slate-800/50 border border-slate-700 rounded-xl hover:bg-slate-800 transition-colors cursor-pointer group"
               >
                  <div className="flex items-center gap-3">
                     <FileText size={18} className="text-slate-500 group-hover:text-emerald-400 transition-colors" />
               ...
               <div 
                  onClick={() => alert("Simulação: Abrindo Rascunho de Artigo.md")}
                  className="flex items-center justify-between p-4 bg-slate-800/50 border border-slate-700 rounded-xl hover:bg-slate-800 transition-colors cursor-pointer group"
               >
                  <div className="flex items-center gap-3">
                     <FileText size={18} className="text-slate-500 group-hover:text-emerald-400 transition-colors" />
