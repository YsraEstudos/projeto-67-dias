...
export const VisualRoadmapView: React.FC<VisualRoadmapViewProps> = ({
    ...
    // Memoized stats calculation - only recalculates when nodes change
    const { totalNodes, completedNodes, progress } = useMemo(() => {
        const total = nodes.filter(n => n.type !== 'section' && n.type !== 'info').length;
        const completed = nodes.filter(n => n.isCompleted && n.type !== 'section' && n.type !== 'info').length;
        return {
            totalNodes: total,
    ...
    // Memoized bounds calculation - expensive Math.min/max operations cached
    ...
    // Memoized width, height, scale derived from bounds
    const { width, height, scale } = useMemo(() => {
        const w = bounds.maxX - bounds.minX;
        const h = bounds.maxY - bounds.minY;
        return { width: w, height: h, scale: Math.min(1, 400 / w, 250 / h) };
    }, [bounds]);
