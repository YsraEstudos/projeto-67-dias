import{f as d,g as w,p as S,I as R,J as h,K as P}from"./ConcursoBootstrap-BXlTwM5Y.js";const l=t=>t.replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#39;"),f=t=>new Intl.DateTimeFormat("pt-BR",{weekday:"short",timeZone:"UTC"}).format(S(t)),g=t=>{const[e,s]=t.split("-").map(Number);return new Intl.DateTimeFormat("pt-BR",{month:"long",year:"numeric",timeZone:"UTC"}).format(new Date(Date.UTC(e,s-1,1)))},b=t=>t.hasSimulado&&t.hasRedacao?"Simulado + Redacao":t.hasSimulado?"Simulado":t.hasRedacao?"Redacao":"Sem evento especial",$=t=>t.planMode==="manual"?`Manual (Semana ${t.weekNumber??"-"})`:"Automatico",y=t=>t.isRestDay?["Domingo de descanso fixo"]:t.planMode==="manual"&&t.manualBlocks&&t.manualBlocks.length>0?t.manualBlocks.map(e=>{const s=e.movedFromSunday?" [realocado do domingo]":"",i=(()=>{const n=R(e)?.replaceAll(" | "," / ");return n?` | Conteúdo programático: ${n}`:""})();return`${e.area}: ${e.title} - ${e.detail}${i}${s}`}):[`Materia: ${h(t.subjects[0])} (bloco principal)`,`Materia: ${h(t.subjects[1])} (bloco principal)`,`Trabalho: ${P(t.workActivity)} (bloco rotativo)`],v=()=>w(),D=(t,e,s)=>{const i=new Blob([t],{type:s}),m=URL.createObjectURL(i),n=document.createElement("a");n.href=m,n.download=e,document.body.append(n),n.click(),n.remove(),URL.revokeObjectURL(m)},k=t=>{const e=[...t].sort((o,c)=>o.date.localeCompare(c.date)),s=e[0]?.date??"",i=e[e.length-1]?.date??"",m=e.filter(o=>o.hasSimulado).length,n=e.filter(o=>o.hasRedacao).length,u=[...new Set(e.map(o=>o.monthKey))],r=["# Plano Completo de Estudos","",`Periodo: ${d(s)} a ${d(i)}`,`Gerado em: ${new Date().toLocaleString("pt-BR")}`,`Simulados planejados: ${m}`,`Redacoes planejadas: ${n}`,""];for(const o of u){r.push(`## ${g(o)}`),r.push("");const c=e.filter(a=>a.monthKey===o);for(const a of c){r.push(`### ${f(a.date)} ${d(a.date)} | ${$(a)} | ${b(a)}`),r.push(`- Meta de questoes objetivas: ${a.targets.objectiveQuestions}`);for(const p of y(a))r.push(`- ${p}`);r.push("")}}return r.join(`
`)},j=t=>{const e=[...t].sort((o,c)=>o.date.localeCompare(c.date)),s=e[0]?.date??"",i=e[e.length-1]?.date??"",m=e.filter(o=>o.hasSimulado).length,n=e.filter(o=>o.hasRedacao).length,r=[...new Set(e.map(o=>o.monthKey))].map(o=>{const c=e.filter(a=>a.monthKey===o).map(a=>{const p=y(a).map(x=>`<li>${l(x)}</li>`).join("");return`
            <article class="day">
  ...
  `},A=t=>{const e=k(t),s=`plano-completo-${v()}.md`;D(e,s,"text/markdown;charset=utf-8")},B=t=>{const e=window.open("","_blank","noopener,noreferrer");if(!e)throw new Error("Popup bloqueado. Permita popups para gerar o PDF.");e.document.write(j(t)),e.document.close(),e.focus(),e.print()};export{B as a,A as e};
